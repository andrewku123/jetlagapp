...
A GeoJSON `FeatureCollection` of `LineString`s; each feature has
`properties = { system, color }`. Generated by
`scripts/fetch_transit_lines.py`, which:
...
3. Colors lines from each route's OSM `colour` tag, except **Caltrain** which is
   collapsed to a single color (`CALTRAIN_COLOR`).
4. Dedupes by OSM way id, processing BART→VTA→Muni→Caltrain so a colored branch
   claims shared track before Caltrain; rounds coords to 5 dp to shrink the file.

Regenerate with: `python3 scripts/fetch_transit_lines.py` (writes the JSON).

## Rendering (`src/components/MapView.tsx`)
`const TRANSIT = transitData` is drawn with a `<GeoJSON data={TRANSIT}
style={transitStyle} interactive={false} />` placed **after** the `<TileLayer>`
and counties overlay but **before** the station markers, so stations sit on top.
`transitStyle(feature)` returns `{ color: feature.properties.color, weight: 3,
