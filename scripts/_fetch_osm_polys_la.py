#!/usr/bin/env python3
"""Fetch OSM footprint polygons for POI categories over the LA play-area bbox.
Writes osm_polys_<cat>_la.json = [{id,name,wkt}, ...]. LA-only helper (the
committed fetch_osm_polys.py uses the Bay Area play_area.geojson)."""
...
# LA bbox derived from the review-data pin extents (+0.02 deg pad)
BBOX = (33.686306699999996, -118.6856559, 34.3461054, -117.7002694)  # S,W,N,E
...
def main():
    ...
    for cat in cats:
        ...
        js = fetch(cat)
        feats = to_polys(js)
        json.dump(feats, open(os.path.join(HERE, f"osm_polys_{cat}_la.json"), "w"))
